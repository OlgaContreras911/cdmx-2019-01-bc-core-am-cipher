window.cipher = {
  ...
  encode: (newValorOffset, newMayus) => {
    ...
    if (newMayus.charCodeAt(i) >= 97 && newMayus.charCodeAt(i) <=  122){
      let formulaCifrado = (newMayus.charCodeAt(i) - 97 + parseInt(newValorOffset)+52) % 26 + 97;
      ...
      let formulaCifrado = (newMayus.charCodeAt(i) - 65 + parseInt(newValorOffset)+52) % 26 + 65;
      let resultadoFinal = String.fromCharCode(formulaCifrado);
      primerCifrado += resultadoFinal;
  ...
  decode: (newValorOffset, newMayus) => {
    ...
    for (let i = 0; i < newMayus.length; i++) {
      if (newMayus.charCodeAt(i) >= 97 && newMayus.charCodeAt(i) <=  122){
        ...
        let formulaCifrado = (newMayus.charCodeAt(i) - 122 - parseInt(newValorOffset)-52) % 26 + 122;
        let resultadoFinal = String.fromCharCode(formulaCifrado);
        primerCifrado += resultadoFinal;
